...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`text-[8px] font-black px-2 py-0.5 rounded-full border uppercase tracking-widest ${styles[status] || styles.pending}`}
    >
...
const ResolutionChat = ({ complaintId, status }) => {
  ...
  return (
    <div className="bg-slate-900/60 rounded-3xl border border-white/5 flex flex-col h-[400px]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
        {msgs.length === 0 ? (
          ...
          msgs.map((m, i) => (
            <div
              key={i}
              className={`flex ${m.senderRole === user.role ? "justify-end" : "justify-start"}`}
            >
              <div
      ...
      {status === "resolved" ? (
        ...
        <form
          onSubmit={onSend}
          className="p-3 border-t border-white/5 bg-white/[0.01] rounded-b-3xl"
        >
          <div className="relative">
            <input
              ...
              className="w-full bg-slate-800/80 border border-white/5 rounded-2xl pl-5 pr-12 py-3.5 text-xs text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all font-bold uppercase tracking-tight"
            />
            <button
              type="submit"
              disabled={loading}
              className="absolute right-2 top-1/2 -translate-y-1/2 p-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-500 transition-all active:scale-95 disabled:opacity-50"
            >
              <Send size={14} />
            </button>
          </div>
...
const ResolutionQueuePage = () => {
  ...
  return (
    <div className="space-y-8 animate-fade-in-up pb-20">
      ...
      {loading ? (
        ...
        <div className="glass-card rounded-[2.5rem] p-20 text-center border-dashed border-2 border-white/5">
          ...
          <p className="text-slate-500 text-sm">
            No institutional grievances currently require resolution authority.
          </p>
        </div>
      ) : (
          ...
          {filtered.map((c) => (
            ...
            >
              <div className="flex items-center gap-6">
                ...
                <div>
                  <h4 className="font-bold text-white text-base group-hover:text-indigo-400 transition-colors uppercase tracking-tight">
                    {c.title}
                  </h4>
      ...
      {/* Case Detail Modal */}
      {selected && (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in pt-10 pb-10">
          <div className="glass-card w-full max-w-2xl rounded-[3rem] p-8 sm:p-10 shadow-2xl border border-white/10 animate-fade-in-up relative overflow-hidden h-fit mb-10">
            ...
            <div className="flex justify-between items-start mb-8 relative z-10 font-bold">
              <div>
                <p className="text-[10px] text-indigo-400 uppercase tracking-[0.25em] mb-1">
                  Signal ID: #{selected._id.slice(-8).toUpperCase()}
                </p>
            ...
            <div className="space-y-6 relative z-10 font-bold">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800/40 p-4 rounded-3xl border border-white/5">
                  <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1.5 items-center flex gap-1.5">
              ...
              {/* Toggle Chat Tab */}
              <div className="flex gap-2 p-1.5 bg-slate-950/40 rounded-2xl border border-white/5 backdrop-blur-sm">
                <button
                  onClick={() => setShowChat(false)}
                  className={`flex-1 py-3 px-4 rounded-xl text-[10px] uppercase tracking-[0.2em] font-black transition-all flex items-center justify-center gap-2 ${!showChat ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/20" : "text-slate-500 hover:text-white bg-white/[0.03] hover:bg-white/5"}`}
                >
                ...
                <>
                  <div className="bg-slate-800/20 p-6 rounded-[2rem] border border-white/5 space-y-3">
                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest flex items-center gap-1.5">
                      <FileText size={12} /> Captured Signal
                    </p>
                  ...
                  {selected.status !== "resolved" && (
                    <div className="flex flex-col sm:flex-row gap-3 pt-4">
                      ...
                      <button
                        onClick={() =>
                          updateStatus(selected._id, "in-progress")
                        }
                        disabled={updating}
                 ...
                 <>
                   <ResolutionChat complaintId={selected._id} status={selected.status} />
                   <div className="h-4" />
                 </>
              )}
              ...
              {selected.status === "resolved" && !showChat && (
                <div className="bg-emerald-500/5 border border-emerald-500/10 p-6 rounded-[2rem] flex items-start gap-4">
                  ...
                  <div>
                    <h5 className="text-emerald-400 font-black text-xs uppercase tracking-widest mb-1.5">
                      Authority Logged Success
                    </h5>
                    <p className="text-[10px] text-slate-500 leading-relaxed font-bold uppercase">
                      This case has been definitively resolved. The remarks
                      below are published to the messenger's terminal.
                    </p>

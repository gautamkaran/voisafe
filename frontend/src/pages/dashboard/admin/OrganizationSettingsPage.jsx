...
const inputCls =
  "w-full bg-slate-900 border border-white/5 text-white placeholder:text-slate-700 rounded-2xl px-5 py-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all font-medium";
...
const OrganizationSettingsPage = () => {
  ...
  const fetchMembers = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("voisafe_token");
      const res = await fetch(`${API_URL}/admin/my-organization/members`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 animate-fade-in-up pb-10">
        ...
        <div className="flex items-center gap-6 relative z-10">
          ...
          <div>
            <p className="text-xl font-black text-white tracking-tight uppercase">
              {orgForm.name || "INSTITUTION"}
            </p>
            <p className="text-xs text-slate-500 font-mono mt-0.5">
              {orgForm.domain || "no-domain-set"}
            </p>
      ...
      <form
        ...
        className="glass-card rounded-[2.5rem] p-8 sm:p-10 border border-white/5 shadow-2xl space-y-6"
      >
        <SectionHeader
          icon={Building2}
          title="Institutional Profile"
          desc="Core identity parameters of the organization."
        />
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-1.5 ml-1">
                Institution Name
              </label>
              <input
                type="text"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setOrgForm((p) => ({ ...p, contactEmail: e.target.value }))
                }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                placeholder="https://institution.ac.in"
                className={inputCls}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setOrgForm((p) => ({ ...p, linkedin: e.target.value }))
                }
      ...
      <div className="glass-card rounded-[2.5rem] p-8 sm:p-10 border border-white/5 shadow-2xl">
        <SectionHeader
          ...
          desc="All users registered under this institution."
        />
        {membersLoading ? (
          <div className="flex items-center gap-2 text-slate-400">
            <RefreshCw className="animate-spin" size={16} /> Loading members...
          </div>
        ) : members.length === 0 ? (
          <div className="text-slate-500 text-sm">No members found.</div>
        ) : (
          <div className="divide-y divide-white/5">
            {members.map((m) => (
              <div key={m._id} className="flex items-center gap-4 py-3">
                <div className="w-8 h-8 rounded-full bg-indigo-500/10 flex items-center justify-center text-indigo-400 font-black text-lg">
                  {m.name?.charAt(0) || "?"}
                </div>
                ...
                <div className="text-[10px] font-black uppercase px-2 py-1 rounded bg-white/10 text-slate-300 border border-white/10">
                  {m.role}
                </div>
              </div>
            ))}
      ...
      <div className="glass-card rounded-[2.5rem] p-8 sm:p-10 border border-white/5 shadow-2xl">
        <SectionHeader
          ...
          desc="Choose what events trigger system notifications."
        />
          ...
          {[
            {
              ...
              desc: "Alert when a new grievance identity is registered.",
            },
            {
              key: "resolved",
              label: "Resolution Success",
              desc: "Notify when a committee marks a case as resolved.",
            },
            {
              key: "escalated",
              label: "Critical Escalations",
              desc: "Alert when a case bypasses standard resolution.",
            },
            {
              key: "weeklyDigest",
              label: "Institutional Digest",
              desc: "Receive a weekly summary of all trust metrics.",
            },
          ].map((n) => (
            <div
              key={n.key}
              className="flex items-center justify-between py-5 border-b border-white/5 last:border-none gap-6 hover:bg-white/[0.01] transition-colors rounded-xl px-2"
            >
              <div>
      ...
      <div className="glass-card rounded-[2.5rem] p-8 sm:p-10 border border-rose-500/10 shadow-2xl bg-rose-500/[0.02]">
        <SectionHeader
          icon={AlertTriangle}
          title="Danger Zone"
          desc="High-risk institutional operations."
        />
        <div className="space-y-4">
          <button
            onClick={() =>
              toast.error("Contact Super Admin for terminal decommissioning.")
            }
          ...
          <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest text-center">
            Irreversible action. Requires Super Admin cryptographic consent.
          </p>
        </div>
      </div>
    </div>

...
const ConfirmDialog = ({ config, onConfirm, onCancel }) => {
  ...
  const isUnblock =
    config.action === "approve" && config.currentStatus === "rejected";
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-slate-900 border border-white/10 rounded-[2rem] shadow-2xl overflow-hidden animate-fade-in-up">
        ...
        <div className="flex flex-col items-center text-center p-8 pb-4">
          <div
            className={`w-16 h-16 rounded-3xl flex items-center justify-center mb-5 ${m.iconCls}`}
          >
            <m.icon size={32} />
          </div>
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 custom-scrollbar space-y-2">
          ...
          <div className="divide-y divide-white/0">
            ...
            <DetailField
              icon={Code2}
              label="College Code"
              value={org.collegeCode}
            />
            <DetailField icon={Globe} label="Domain" value={org.domain} />
            <DetailField
              icon={Info}
              label="Type"
              value={org.typeOfOrg?.toUpperCase()}
            />
            <DetailField icon={MapPin} label="Address" value={org.address} />
            <DetailField
              icon={LinkIcon}
              label="Website"
              value={org.socialMediaLink?.website}
            />
            <DetailField
              icon={LinkIcon}
              label="LinkedIn"
              value={org.socialMediaLink?.linkedin}
            />
            <DetailField
              icon={LinkIcon}
              label="Twitter / X"
              value={org.socialMediaLink?.twitter}
            />
            <DetailField
              icon={CalendarDays}
              label="Registered On"
        ...
        <div className="px-6 py-5 border-t border-white/5 flex items-center gap-3 flex-wrap shrink-0">
          ...
          <button
            onClick={onClose}
            className="py-3 px-5 bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white text-sm font-bold rounded-2xl transition-all"
          >
            Close
          </button>
        </div>
      ...
      <div className="space-y-10 animate-fade-in-up">
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div className="space-y-2">
              ...
              <p className="text-slate-400 text-sm max-w-md font-medium leading-relaxed">
                Approve registrations, inspect institution profiles, and enforce
                access policies.
              </p>
            </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {globalStats.map((stat) => (
            <div
              key={stat.name}
              className="glass-card rounded-3xl p-5 sm:p-7 flex items-center justify-between group hover:border-indigo-500/30 transition-all shadow-xl"
            >
              <div className="min-w-0">
                <p className="text-slate-500 text-[9px] sm:text-xs font-bold uppercase tracking-widest mb-1 truncate">
                  {stat.name}
                </p>
              ...
              <div
                className={`p-2.5 sm:p-3.5 ${stat.bg} rounded-2xl group-hover:scale-110 transition-transform`}
              >
          ...
          {pendingOrgs > 0 && (
            <div className="flex items-center gap-4 p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl">
              ...
              <p className="text-sm text-amber-300 font-semibold">
                <strong>
                  {pendingOrgs} institution
                  {pendingOrgs > 1 ? "s require" : " requires"}
                </strong>{" "}
                your approval.
              </p>
            </div>
